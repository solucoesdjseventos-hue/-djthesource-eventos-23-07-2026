...
const createTransporter = () => {
  ...
  if (!host || !port || !user || !pass) {
    throw new Error('SMTP configuration is required. Please set SMTP_HOST, SMTP_PORT, SMTP_USER, and SMTP_PASS in .env');
  }
...
const sendQuote = async (req, res) => {
  try {
    const { clientName, clientEmail, clientPhone, organizerEmail, quote } = req.body;
    if (!organizerEmail || !quote || !clientName || !clientEmail) {
      return res.status(400).json({ error: 'organizerEmail, clientName, clientEmail and quote are required' });
    }

    // persist quote to SQLite DB
    try {
      ...
      console.error('Erro ao salvar orçamento no DB:', storeErr);
    }
    ...
    return res.status(500).json({ error: error.message || 'Erro no servidor ao enviar email' });
  }
};
...
module.exports = { sendQuote, listQuotes };
